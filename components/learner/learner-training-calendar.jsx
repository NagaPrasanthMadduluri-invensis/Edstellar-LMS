...
const STATUS_CFG = {
  upcoming:  { label: "Upcoming",  cls: "bg-amber-100 text-amber-700 border-0"    },
...
const ATTENDANCE_CFG = {
  present:  { label: "Present",  cls: "bg-emerald-100 text-emerald-700 border-0" },
  absent:   { label: "Absent",   cls: "bg-red-100 text-red-600 border-0"         },
  late:     { label: "Late",     cls: "bg-blue-100 text-blue-700 border-0"       },
  partial:  { label: "Partial",  cls: "bg-purple-100 text-purple-700 border-0"   },
  excused:  { label: "Excused",  cls: "bg-orange-100 text-orange-700 border-0"   },
};
...
export function LearnerTrainingCalendar() {
  ...
  useEffect(() => {
    if (!token) return;
    apiClient("/api/learner/sessions", { token })
      .then((d) => {
                ...
                >
                  {day && (
                    <>
                      ...
                      >
                        {day}
                        {todayCell && <Text as="span" className="ml-1 text-xs text-blue-500 font-semibold">· Today</Text>}
                      </Text>
              ...
              >
                <Box className="flex items-start justify-between gap-4 flex-wrap">
                  <Box className="flex-1 min-w-0 space-y-1.5">
                    ...
                    {s.course_name && (
                      <Box className="flex items-center gap-1 text-xs text-indigo-600">
