...
const STRENGTH_COLORS = ["", "bg-error", "bg-navy", "bg-navy", "bg-navy"];
const STRENGTH_TEXT   = ["", "text-error", "text-ink/70", "text-ink/70", "text-navy"];
  ...
  if (success) {
    return (
      <Card className="max-w-md mx-auto p-8 flex flex-col items-center gap-4 text-center">
        <Box className="w-16 h-16 rounded-full bg-paper-cream flex items-center justify-center">
    ...
    <Card className="max-w-md mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Box className="space-y-1.5">
          ...
          />
          {fieldErrors.current && (
            <Text as="p" className="text-xs text-error">{fieldErrors.current}</Text>
          )}
        </Box>
              ...
              return (
                <Box key={rule.key} className="flex items-center gap-2">
                  <Box className={cn(
                    ...
                    met   ? "bg-navy" :
                    unmet ? "bg-error/10"     : "bg-muted"
                  )}>
                    {met   && <Check className="h-2.5 w-2.5 text-white" />}
                    {unmet && <X     className="h-2.5 w-2.5 text-error" />}
                  </Box>
                  <Text as="span" className={cn(
                    "text-xs",
                    met   ? "text-navy font-medium" :
                    unmet ? "text-error"                  : "text-muted-foreground"
                  )}>
                    {rule.label}
                  </Text>
                </Box>
            ...
            <Box className="flex items-center gap-1.5 pt-0.5">
              {matches ? (
                <>
                  ...
                  <Text as="span" className="text-xs text-navy font-medium">Passwords match</Text>
                </>
              ) : (
                <>
                  <X className="h-3.5 w-3.5 text-error" />
                  <Text as="span" className="text-xs text-error">Passwords do not match</Text>
                </>
              )}
            </Box>
          )}
        ...
        {/* Global error */}
        {error && (
          <Box className="rounded-lg bg-error/10 border border-error/30 px-4 py-3">
