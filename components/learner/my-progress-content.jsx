...
/* ── Status config ── */
const STATUS_CFG = {
  "not started": { label: "not started", cls: "bg-gray-100 text-gray-500 border-gray-200" },
  "in progress":  { label: "in progress",  cls: "bg-indigo-50 text-indigo-600 border-indigo-200" },
...
const TYPE_CFG = {
  ...
  SCORM: { label: "SCORM", cls: "bg-green-100 text-green-700" },
};
...
function ScoreCell({ score, passed }) {
  ...
  const cls = passed === false ? "text-red-500" : passed === true ? "text-emerald-600" : "text-amber-600";
...
export function MyProgressContent() {
  ...
  const statCards = [
    { icon: Layers,    iconBg: "bg-blue-100",    iconColor: "text-blue-600",    circleBg: "bg-blue-50",    value: summary.assigned,        label: "Courses Assigned"  },
    { icon: CheckCircle2, iconBg: "bg-emerald-100", iconColor: "text-emerald-600", circleBg: "bg-emerald-50", value: summary.completed,    label: "Completed"         },
    { icon: TrendingUp,iconBg: "bg-green-100",   iconColor: "text-green-600",   circleBg: "bg-green-50",   value: `${summary.completionRate}%`, label: "Completion Rate" },
    { icon: Award,     iconBg: "bg-orange-100",  iconColor: "text-orange-500",  circleBg: "bg-orange-50",  value: summary.avgScore !== null ? `${summary.avgScore}%` : "—", label: "Avg Score" },
    { icon: Star,      iconBg: "bg-amber-100",   iconColor: "text-amber-500",   circleBg: "bg-amber-50",   value: summary.bestScore !== null ? `${summary.bestScore}%` : "—", label: "Best Score" },
    { icon: Clock,     iconBg: "bg-pink-100",    iconColor: "text-pink-500",    circleBg: "bg-pink-50",    value: `${summary.allTimeHours}h`, label: "All-Time Hours" },
  ];
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {courseHistory.map((c, i) => {
                    ...
                    const barCls = c.status === "failed" ? "bg-red-500"
        ...
        <Card className="p-5">
          ...
          {skills.length === 0 ? (
            <Box className="flex flex-col items-center justify-center py-12 gap-3">
              <Box className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center">
            ...
            <Box className="flex flex-wrap gap-2 mt-2">
              {skills.map((s) => (
                <Badge key={s} variant="secondary" className="text-sm px-3 py-1 bg-indigo-50 text-indigo-700 border-indigo-200 border">
                  {s}
                </Badge>
              ))}
      ...
      <Box className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {assessmentPerformance.attempts.length === 0 ? (
            ...
            <Box className="space-y-4">
              ...
              <Box className="grid grid-cols-3 gap-4 pb-4 border-b">
                {[
                  { val: assessmentPerformance.avgScore,  label: "AVG SCORE",  cls: assessmentPerformance.avgScore  !== null && assessmentPerformance.avgScore  < 50 ? "text-red-500" : "text-emerald-600" },
                  { val: assessmentPerformance.bestScore, label: "BEST SCORE", cls: "text-emerald-600" },
                  { val: assessmentPerformance.passRate,  label: "PASS RATE",  cls: assessmentPerformance.passRate !== null && assessmentPerformance.passRate  < 50 ? "text-red-500" : "text-emerald-600" },
                ].map(({ val, label, cls }) => (
              ...
              <Box className="space-y-3">
                {assessmentPerformance.attempts.map((a) => (
                  <Box key={`${a.courseName}::${a.assessmentTitle}`} className="flex items-center gap-3">
                    ...
                    <Text as="span" className={cn("text-xs font-bold w-8 text-right shrink-0", a.passed ? "text-emerald-600" : "text-red-500")}>{a.score}%</Text>
                    <Badge variant="outline" className={cn("text-[10px] shrink-0", a.passed ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-red-50 text-red-600 border-red-200")}>
                      {a.passed ? "Pass" : "Fail"}
                    </Badge>
                  </Box>
                ))}
        ...
        <Card className="p-5">
          ...
          <Box className="grid grid-cols-3 gap-4 mb-5">
            {[
              { val: `${learningHours.thisMonth}h`, label: "THIS MONTH", cls: "text-blue-600" },
              { val: `${learningHours.lastMonth}h`, label: "LAST MONTH", cls: "text-blue-600" },
              { val: `${learningHours.allTime}h`,   label: "ALL TIME",   cls: "text-blue-600" },
            ].map(({ val, label, cls }) => (
              <Box key={label} className="text-center">
          ...
          <Box className={cn(
            ...
            diffPositive ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
              : diffNeutral ? "bg-gray-50 text-gray-600 border border-gray-200"
          ...
          <Box>
            <Box className="flex items-center justify-between mb-1.5">
              ...
              <Text as="p" className={cn("text-xs font-bold", learningHours.goalPct >= 100 ? "text-emerald-600" : "text-amber-600")}>
            ...
            <Box className="h-2.5 bg-muted rounded-full overflow-hidden">
              <Box
                ...
                style={{
                  ...
                  background: learningHours.goalPct >= 100 ? "#10b981" : "#f59e0b",
                }}
              />
            </Box>
          </Box>
