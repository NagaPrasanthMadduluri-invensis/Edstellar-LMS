...
function ScormLessonView({ lesson, courseId, progressStatus, onStatusRefresh }) {
  ...
  const loadTracking = useCallback(async () => {
    if (!token || !lesson.scorm_package_id) return;
    try {
      const res = await fetch(`/api/learner/scorm/${lesson.scorm_package_id}/tracking`, {
  ...
  }, [lesson.scorm_package_id, token]);

  useEffect(() => { if (token) loadTracking(); }, [token, loadTracking]);
    ...
    <Box className="space-y-4">
      ...
      {tracking === undefined ? (
          ...
          <Box className="flex items-start justify-between gap-4 flex-wrap">
            <Box className="space-y-3 flex-1 min-w-0">
              <Box className="flex items-center gap-2 flex-wrap">
                ...
                {isFailed && (
                  <Badge className="text-[10px] bg-red-100 text-red-700 border-0">Failed</Badge>
                )}
        ...
        <Card className="p-5 text-center border-dashed">
          <FileArchive className="h-10 w-10 mx-auto text-indigo-400/50 mb-2" />
...
export function LessonContent({ courseId, lessonId }) {
  ...
  const loadLesson = useCallback(async () => {
    ...
    try {
      const res = await fetchLessonContent({ token, lessonId });
      setData(res);
          ...
          <Box className="flex items-start justify-between gap-4 flex-wrap">
            <Box className="flex-1 min-w-0">
              <Box className="flex items-center gap-2 mb-2">
                {lessonTypeIcon}
                <Badge
                  className={`text-[10px] font-semibold px-2 py-0.5 ${status === "completed" ? "bg-emerald-100 text-emerald-700 border-emerald-200" : isScorm ? "bg-indigo-100 text-indigo-700 border-indigo-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}
                >
            ...
            {!isScorm && (
              status === "completed" ? (
                <Button size="sm" disabled className="shrink-0 bg-emerald-600 text-white opacity-70">
