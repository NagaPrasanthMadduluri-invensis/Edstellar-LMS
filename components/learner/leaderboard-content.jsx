...
function PodiumMedal({ pos }) {
  if (pos === 1) return (
    <Box className="w-7 h-7 rounded-full bg-amber-400 border-2 border-amber-500 flex items-center justify-center text-white text-xs font-extrabold shadow-md">1</Box>
  );
  if (pos === 2) return (
    <Box className="w-7 h-7 rounded-full bg-gray-400 border-2 border-gray-500 flex items-center justify-center text-white text-xs font-extrabold shadow-md">2</Box>
  );
  return (
    <Box className="w-7 h-7 rounded-full bg-amber-700 border-2 border-amber-800 flex items-center justify-center text-white text-xs font-extrabold shadow-md">3</Box>
  );
}

const PODIUM_CFG = {
  1: { border: "border-amber-300", bg: "bg-amber-50", pointsColor: "text-amber-600", ribbonColor: "#f59e0b", height: "h-36" },
  2: { border: "border-gray-200",  bg: "bg-gray-50",  pointsColor: "text-gray-500",  ribbonColor: "#9ca3af", height: "h-28" },
  3: { border: "border-amber-200", bg: "bg-orange-50",pointsColor: "text-orange-500",ribbonColor: "#f97316", height: "h-24" },
};

const RECOGNITION_CFG = {
  learnerOfMonth:   { icon: "🌟", label: "Learner of the Month",  border: "border-l-amber-400",  iconBg: "bg-amber-50", tag: "text-amber-600" },
  quickLearner:     { icon: "⚡", label: "Quick Learner",          border: "border-l-purple-400", iconBg: "bg-purple-50",tag: "text-purple-600" },
  assessmentTopper: { icon: "🎯", label: "Assessment Topper",      border: "border-l-red-400",    iconBg: "bg-red-50",   tag: "text-red-600"   },
};
...
export function LeaderboardContent() {
  ...
  useEffect(() => {
    ...
    fetch("/api/learner/leaderboard", { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
  ...
  return (
    <Box className="space-y-5">
      ...
      {/* ── Hero banner ── */}
      <Box className="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-5 flex items-center gap-4">
                ...
                {rec ? (
                  <>
                    <Box className="flex items-center gap-1.5">
                      ...
                      {rec.isYou && <Badge className="text-[9px] px-1.5 py-0 bg-blue-500 text-white border-0 shrink-0">You</Badge>}
                    </Box>
            ...
            <Box key={p.id} className="flex flex-col items-center gap-2 w-36">
              ...
              <Box className="text-center">
                <Box className="flex items-center justify-center gap-1">
                  ...
                  {p.isYou && <Badge className="text-[9px] px-1 py-0 bg-blue-500 text-white border-0">You</Badge>}
                </Box>
          ...
          <Box className="flex items-center gap-2 flex-wrap">
            ...
            <Box className="flex rounded-lg border overflow-hidden text-xs">
              <button
                ...
                className={cn("px-4 py-1.5 font-semibold transition-colors", period === "alltime" ? "bg-blue-600 text-white" : "bg-white text-muted-foreground hover:bg-muted")}
              >
                All-time
              </button>
              <button
                ...
                className={cn("px-4 py-1.5 font-semibold transition-colors", period === "month" ? "bg-blue-600 text-white" : "bg-white text-muted-foreground hover:bg-muted")}
              >
                This Month
              </button>
            </Box>
            ...
            <select
              ...
              className="text-xs border rounded-lg px-3 py-1.5 bg-white text-foreground focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              {departments.map((d) => (
              ...
              {filteredRankings.map((l) => (
                <tr
                  ...
                  className={cn(
                    ...
                    l.isYou ? "bg-blue-50/70" : "hover:bg-muted/20"
                  )}
                >
                  ...
                  <td className="py-3 px-3">
                    <Box className="flex items-center gap-3">
                      ...
                      <Box className="flex items-center gap-1.5 flex-wrap">
                        <Text as="p" className={cn("text-sm font-semibold", l.isYou && "text-blue-700")}>{l.name}</Text>
                        {l.isYou && (
                          <Badge className="text-[9px] px-1.5 py-0 bg-blue-500 text-white border-0 shrink-0">You</Badge>
                        )}
                      </Box>
                    </Box>
                  </td>
                  ...
                  <td className="py-3 pl-3 pr-5">
                    <Text as="p" className={cn("text-sm font-bold", l.isYou ? "text-blue-600" : "text-blue-500")}>
