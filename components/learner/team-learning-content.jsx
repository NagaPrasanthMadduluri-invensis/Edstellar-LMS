  ...
  return (
    <Box className="space-y-5">
        ...
        <Box className="flex items-start justify-between gap-4 flex-wrap">
          <Box>
            ...
            <Box className="flex items-center gap-1.5 mt-1 flex-wrap text-sm text-muted-foreground">
              <Text as="span">
                <Text as="span" className={cn("font-semibold", completionPct > 0 ? "text-emerald-600" : "text-orange-500")}>
                  {completionPct}%
          ...
          <Box className="flex items-center gap-2 flex-wrap">
            {attention.length > 0 && (
              <Badge className="bg-orange-50 text-orange-600 border border-orange-200 gap-1.5 px-3 py-1.5 text-xs font-medium">
      ...
      <Box className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard icon={Users2}    iconBg="bg-blue-50 text-blue-500"    value={totalSize}           label="Team Size"      />
        <StatCard icon={CheckCircle2} iconBg="bg-emerald-50 text-emerald-500" value={completed}     label="Completed"      />
        <StatCard icon={TrendingUp} iconBg="bg-indigo-50 text-indigo-500" value={inProgress}        label="In Progress"    />
        <StatCard icon={Clock}     iconBg="bg-amber-50 text-amber-500"   value={notStarted}         label="Not Started"    />
        <StatCard icon={Trophy}    iconBg="bg-violet-50 text-violet-500" value={`${avgScore}%`}     label="Avg Score"      />
        <StatCard icon={Timer}     iconBg="bg-pink-50 text-pink-500"     value={`${avgHrs}h`}       label="Avg Hrs/Month"  />
      </Box>
        ...
        <Card className="p-5">
          ...
          <Box className="flex items-center gap-8">
            <Box className="text-center shrink-0">
              <Text as="p" className={cn("text-4xl font-extrabold", completionPct > 0 ? "text-emerald-600" : "text-orange-500")}>
                {completionPct}%
              </Text>
            ...
            <Box className="space-y-2.5 flex-1">
              {[
                { dot: "bg-emerald-500", label: "Completed",   count: completed   },
                { dot: "bg-blue-400",    label: "In Progress",  count: inProgress  },
                { dot: "bg-gray-300",    label: "Not Started",  count: notStarted  },
                { dot: "bg-red-500",     label: "Failed",       count: failed      },
              ].map(({ dot, label, count }) => (
          ...
          {TEAM_MEMBERS.map((m) => (
            ...
            >
              ...
              <Box className="flex items-center gap-3">
                ...
                <Box>
                  <Text as="p" className="text-sm font-semibold text-indigo-600 cursor-pointer hover:underline">{m.name}</Text>
              ...
              <Box className="flex items-center gap-2">
                <Box className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                  <Box
                    className={cn("h-full rounded-full transition-all", m.progress === 100 ? "bg-emerald-500" : "bg-blue-400")}
              ...
              <Box className="flex items-center gap-1.5">
                <Box className="w-16 h-1.5 rounded-full bg-muted overflow-hidden">
                  <Box
                    className={cn(
                      "h-full rounded-full",
                      m.hours >= m.goal ? "bg-emerald-500" : m.hours > 0 ? "bg-amber-400" : "bg-red-300"
                    )}
                    style={{ width: `${Math.min(100, (m.hours / m.goal) * 100)}%` }}
                  />
                </Box>
                <Text as="span" className={cn("text-xs font-semibold", m.hours >= m.goal ? "text-emerald-600" : "text-red-500")}>
                  {m.hours}h
                </Text>
          ...
          {TEAM_MEMBERS.map((m) => {
            ...
            return (
              <Box key={m.id} className="flex items-center gap-3">
                ...
                <Text as="span" className={cn("text-xs font-semibold w-14 text-right", m.hours >= m.goal ? "text-emerald-600" : "text-red-500")}>
                  {m.hours}h/{m.goal}h
                </Text>
              </Box>
            );
