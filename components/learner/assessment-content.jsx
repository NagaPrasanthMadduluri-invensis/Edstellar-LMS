  ...
  return (
    <Box className="space-y-5">
      ...
      <Card className={`overflow-hidden`}>
        <Box className={`h-2 ${passed ? "bg-navy" : "bg-navy"}`} />
        <CardContent className="p-6 text-center">
          <Box className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-4 ${passed ? "bg-paper-cream" : "bg-error/10"}`}>
            {passed
      ...
      {passed ? (
                        ...
                        return (
                          <Box
                            ...
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
                              isCorrectOpt ? "bg-paper-cream text-navy font-medium" :
                              isSelected && !isCorrectOpt ? "bg-error/10 text-error" :
                              "bg-muted/30 text-muted-foreground"
                            }`}
                          >
                            {isCorrectOpt && <CheckCircle2 className="h-3.5 w-3.5 text-navy shrink-0" />}
                            {isSelected && !isCorrectOpt && <XCircle className="h-3.5 w-3.5 text-error shrink-0" />}
        ...
        <Card className="p-6 bg-paper-cream border-border">
          <Box className="flex flex-col items-center text-center gap-3">
            <Box className="w-14 h-14 rounded-full bg-paper-cream flex items-center justify-center">
              <RotateCcw className="h-7 w-7 text-ink/70" />
            </Box>
            <Text as="h3" className="text-base font-semibold text-ink/70">Don&apos;t give up!</Text>
            <Text as="p" className="text-sm text-ink/70 max-w-md">
...
export function AssessmentContent({ courseId, assessmentId }) {
  ...
  const loadData = () => {
    ...
    fetchAssessmentAttempts({ assessmentId })
      .then((d) => setAttempts(d.attempts || []))
      .catch(() => {});
  };

  useEffect(() => { loadData(); }, [user, user, assessmentId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await submitAssessmentAttempt({ assessmentId, answers: answerList });
  ...
  return (
    <Box className="space-y-5">
        ...
        <CardContent className="p-5">
          <Box className="flex items-start justify-between gap-4 flex-wrap">
            <Box className="flex items-start gap-3">
              <Box className="w-11 h-11 rounded-xl bg-paper-cream flex items-center justify-center shrink-0">
            ...
            <Box className="text-right shrink-0">
              <Text as="p" className="text-2xl font-bold text-navy">{answered}/{questions.length}</Text>
          ...
          <Box className="flex items-center gap-2 flex-wrap">
            {attempts.map((att, i) => (
              <Box key={att.id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted/50 text-xs">
                ...
                <Badge className={`text-[10px] ${att.is_passed ? "bg-paper-cream text-navy" : "bg-error/10 text-error"}`}>
                  {att.percentage}%
                </Badge>
              </Box>
            ))}
      ...
      <Box className="space-y-4">
        {questions.map((q, idx) => (
          <Card key={q.id} className={`transition-all ${answers[q.id] ? "border-navy/20 shadow-sm" : ""}`}>
            <CardContent className="p-5">
              <Box className="flex items-start gap-3 mb-4">
                <Box className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${answers[q.id] ? "bg-navy text-white" : "bg-muted text-muted-foreground"}`}>
                  {idx + 1}
                </Box>
              ...
              >
                {q.options.map((opt) => (
                  <Box key={opt.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all hover:border-navy/20 hover:bg-paper-cream ${String(answers[q.id]) === String(opt.id) ? "border-navy/20 bg-paper-cream" : "border-border"}`}>
