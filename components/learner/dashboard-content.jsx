...
const STATUS_CFG = {
  ...
  "assigned":    { label: "Assigned",    cls: "bg-paper-warm text-ink/60 border-border"   },
  "completed":   { label: "Completed",   cls: "bg-navy text-paper border-navy" },
};

const ACTIVITY_ICON = {
  lesson:     { icon: Play,          bg: "bg-paper-cream",    color: "text-navy"    },
  assessment: { icon: CheckCircle2,  bg: "bg-paper-cream", color: "text-navy" },
  assignment: { icon: ClipboardList, bg: "bg-paper-cream",   color: "text-ink/70"   },
};
  ...
  const statCards = [
    {
      ...
      icon: Package,
      iconBg: "bg-paper-cream", iconColor: "text-navy", circle: "bg-paper-cream",
    },
    {
      ...
      sub: "+ Active courses",
      icon: TrendingUp,
      iconBg: "bg-paper-cream", iconColor: "text-ink/70", circle: "bg-paper-cream",
    },
    {
        ...
        <Box className="flex items-center justify-between gap-4 flex-wrap">
          <Box>
            ...
            <Text as="p" className="text-sm text-muted-foreground mt-0.5">
              {inProgressCount > 0
                ? <>You have <Text as="span" className="text-navy font-semibold">{inProgressCount} course{inProgressCount !== 1 ? "s" : ""} in progress</Text>{yetToStart > 0 ? <> and <Text as="span" className="text-navy font-semibold">{yetToStart} assigned</Text> and waiting</> : ""}.</>
                : <>You have <Text as="span" className="text-navy font-semibold">{stats.assigned_courses ?? 0} course{(stats.assigned_courses ?? 0) !== 1 ? "s" : ""} assigned</Text>. Start learning today!</>}
            </Text>
          </Box>
          {skill_tags.length > 0 && (
            <Box className="flex items-center gap-2 flex-wrap">
              {skill_tags.map((tag) => (
                <Text key={tag} as="span" className="px-3 py-1 text-xs border border-border rounded-full text-ink/60 bg-white">
                  {tag}
                </Text>
              ))}
            </Box>
            ...
            {continue_learning ? (
              <Box className="space-y-3">
                <Box className="flex items-start gap-3">
                  <Box className="w-10 h-10 rounded-lg bg-paper-cream flex items-center justify-center shrink-0 mt-0.5">
                ...
                <Box className="space-y-1.5">
                  <Box className="flex items-center justify-between">
                    <Box className="flex-1 h-2 bg-muted rounded-full overflow-hidden mr-3">
                      <Box className="h-full bg-navy rounded-full" style={{ width: `${continue_learning.progress_percentage}%` }} />
                    </Box>
                    ...
                    return (
                      <Link key={c.course_id} href={`/my-courses/${c.course_id}`}>
                        <Box className={cn(
                          ...
                          {c.status === "completed" ? (
                            <Box className="w-6 h-6 rounded-full bg-navy flex items-center justify-center shrink-0">
                              <CheckCircle2 className="h-3.5 w-3.5 text-white" />
                            </Box>
                          ) : (
                            <Box className={cn(
                              "w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-xs font-bold",
                              isCurrent ? "bg-navy text-white" : "bg-muted text-muted-foreground"
                            )}>
                              {i + 1}
                            </Box>
                          )}
                          <Text as="p" className={cn(
                            ...
                            c.status === "completed" ? "text-navy" : "text-foreground"
                          )}>
                            {c.name}
                          </Text>
                          {isCurrent && (
                            <Badge className="text-[10px] bg-paper-cream text-navy border-0 shrink-0">Current</Badge>
                          )}
                        </Box>
                      </Link>
                    );
