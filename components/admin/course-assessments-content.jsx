...
export function CourseAssessmentsContent({ courseId }) {
  ...
  const loadAssessments = useCallback(async () => {
    ...
    setError(null);
    try {
      const d = await fetchAssessments({ courseId });
      const list = d.assessments || [];
      const withQ = await Promise.all(list.map(async (a) => {
        try {
          const detail = await fetchAssessmentDetail({ assessmentId: a.id });
  ...
  const handleSaveA = async () => {
    ...
    try {
      if (editingA) {
        await updateAssessment({ assessmentId: editingA.id, data: { ...aForm, is_active: editingA.is_active } });
      } else {
        await createAssessment({ courseId, data: aForm });
      }
      setADialog(false);
      loadAssessments();
  ...
  const handleDeleteA = async () => {
    if (!deleteAId) return;
    await deleteAssessment({ assessmentId: deleteAId }).catch(() => {});
    setDeleteAId(null);
    loadAssessments();
  };
  ...
  const handleSaveQ = async () => {
    ...
    try {
      ...
      if (editingQ) {
        await updateQuestion({ questionId: editingQ.id, data: payload });
      } else {
        await addQuestion({ assessmentId: activeAId, data: payload });
      }
      setQDialog(false);
      loadAssessments();
  ...
  const handleDeleteQ = async () => {
    if (!deleteQId) return;
    await deleteQuestion({ questionId: deleteQId }).catch(() => {});
    setDeleteQId(null);
    loadAssessments();
  };
  ...
  return (
    <Box className="space-y-4">
      <Box className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={openCreateA} className="bg-navy hover:bg-navy-soft text-paper h-9">
      ...
      {assessments.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <Button size="sm" onClick={openCreateA} className="mt-3 bg-navy hover:bg-navy-soft text-paper">
            <Plus className="h-4 w-4 mr-1.5" />
            Create Assessment
          </Button>
        </Card>
            ...
            <Card key={a.id} className="overflow-hidden border-l-4 border-l-violet-500">
              <CardHeader className="py-3 px-4 flex flex-row items-center justify-between gap-2">
                <Box className="flex items-center gap-3 flex-1 min-w-0">
                  <Box className="w-10 h-10 rounded-xl bg-paper-cream flex items-center justify-center shrink-0">
                  ...
                  <Box className="flex-1 min-w-0">
                    ...
                    <Box className="flex items-center gap-3 mt-0.5">
                      ...
                      {a.attempts_count > 0 && (
                        <Badge className="text-[10px] border-0 bg-paper-cream text-ink/70">
                ...
                <Box className="flex items-center gap-2 shrink-0">
                  ...
                  <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-error hover:text-error hover:bg-error/10"
                ...
                <Box className="space-y-2 mb-3">
                  {(a.questions || []).map((q, idx) => (
                    <Box key={q.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/30 border group hover:bg-muted/50 transition-colors">
                      <Box className="w-6 h-6 rounded-full bg-paper-cream flex items-center justify-center text-[11px] font-bold text-navy shrink-0 mt-0.5">
                        {idx + 1}
                      </Box>
                      <Box className="flex-1 min-w-0">
                          ...
                          {q.options.map((o) => (
                            <Box
                              ...
                              className={`flex items-center gap-1 text-xs px-2 py-1 rounded-md border ${
                                o.is_correct
                                  ? "bg-paper-cream border-navy/20 text-navy font-semibold"
                      ...
                      <Box className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-error hover:bg-error/10"
          ...
          <Box className="space-y-5 py-2">
            ...
            {aError && (
              <Box className="bg-error/10 border border-error/30 rounded-lg px-3 py-2">
                <Text as="p" className="text-sm text-error">{aError}</Text>
              </Box>
            )}
          </Box>
          <DialogFooter>
            <Button variant="outline" onClick={() => setADialog(false)}>Cancel</Button>
            <Button onClick={handleSaveA} disabled={aSaving} className="bg-navy hover:bg-navy-soft text-paper">
      ...
      <Dialog open={qDialog} onOpenChange={setQDialog}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <Box className="space-y-5 py-2">
            <Box className="space-y-2">
              <Label className="text-sm font-medium">
                Question <Text as="span" className="text-error">*</Text>
              </Label>
              <Textarea
                ...
                {qForm.options.map((opt, i) => (
                  <Box
                    ...
                    className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${
                      opt.is_correct ? "border-navy/20 bg-paper-cream" : "border-border bg-muted/20"
                    }`}
                  >
                    ...
                    <Input
                      ...
                      className={`flex-1 h-9 border-0 bg-transparent shadow-none focus-visible:ring-0 p-0 text-sm ${opt.is_correct ? "font-medium text-navy" : ""}`}
                    />
                    {opt.is_correct && <CheckCircle2 className="h-4 w-4 text-navy shrink-0" />}
                  </Box>
                ))}
              </RadioGroup>
            </Box>
            ...
            {qError && (
              <Box className="bg-error/10 border border-error/30 rounded-lg px-3 py-2">
                <Text as="p" className="text-sm text-error">{qError}</Text>
              </Box>
            )}
          </Box>
          <DialogFooter>
            <Button variant="outline" onClick={() => setQDialog(false)}>Cancel</Button>
            <Button onClick={handleSaveQ} disabled={qSaving} className="bg-navy hover:bg-navy-soft text-paper">
      ...
      <AlertDialog open={!!deleteAId} onOpenChange={(o) => !o && setDeleteAId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteA} className="bg-error hover:bg-error text-white">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!deleteQId} onOpenChange={(o) => !o && setDeleteQId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteQ} className="bg-error hover:bg-error text-white">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </Box>
