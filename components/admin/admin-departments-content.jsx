  ...
  return (
    <Box className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {departments.map((dept, idx) => {
        ...
        const stats = [
          { label: "Completed",   val: dept.completed,   color: "text-emerald-500" },
          { label: "In Progress", val: dept.in_progress, color: "text-blue-500"    },
          { label: "Not Started", val: dept.not_started, color: "text-gray-500"    },
        ...
        return (
          <Card key={dept.dept} className={`border-l-4 ${border} overflow-hidden`}>
            <CardContent className="p-5 space-y-4">
              ...
              <Box className="flex items-start justify-between gap-4 pt-1">

                {/* Stats — plain numbers, no box backgrounds */}
