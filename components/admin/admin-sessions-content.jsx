...
const STATUS_CFG = {
  upcoming:  { label: "Scheduled", cls: "bg-amber-100 text-amber-700 border-0"    },
  completed: { label: "Completed", cls: "bg-emerald-100 text-emerald-700 border-0" },
  cancelled: { label: "Cancelled", cls: "bg-gray-100 text-gray-500 border-0"       },
};

const TYPE_CFG = {
  ILT:     { label: "ILT",     cls: "bg-blue-100 text-blue-700 border-0",    icon: MapPin },
  Virtual: { label: "Virtual", cls: "bg-emerald-100 text-emerald-700 border-0", icon: Video },
  Webinar: { label: "Webinar", cls: "bg-purple-100 text-purple-700 border-0", icon: Video  },
};

const ATTENDANCE_STATUS_CFG = {
  present:  { label: "Present",  cls: "text-emerald-600", triggerCls: "border-emerald-300 bg-emerald-50 text-emerald-700" },
  absent:   { label: "Absent",   cls: "text-red-600",     triggerCls: "border-red-300 bg-red-50 text-red-700"             },
  late:     { label: "Late",     cls: "text-blue-600",    triggerCls: "border-blue-300 bg-blue-50 text-blue-700"           },
  partial:  { label: "Partial",  cls: "text-purple-600",  triggerCls: "border-purple-300 bg-purple-50 text-purple-700"    },
  excused:  { label: "Excused",  cls: "text-orange-600",  triggerCls: "border-orange-300 bg-orange-50 text-orange-700"    },
};
...
function RosterDialog({ sessionId, sessionTitle, open, onClose, onRosterSaved, token }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const d = await apiClient(`/api/admin/sessions/${sessionId}/roster`, { token });
  ...
  const handleAdd = async (userId) => {
    const d = await apiClient(`/api/admin/sessions/${sessionId}/roster`, { method: "POST", token, body: { user_id: userId } });
    setEnrolled(d.enrolled || []);
    setAvailable(d.available || []);
  };

  const handleRemove = async (userId) => {
    const d = await apiClient(`/api/admin/sessions/${sessionId}/roster`, { method: "DELETE", token, body: { user_id: userId } });
    setEnrolled(d.enrolled || []);
    setAvailable(d.available || []);
  };

  const handleEnrollAll = async (dept) => {
    const d = await apiClient(`/api/admin/sessions/${sessionId}/roster`, { method: "POST", token, body: { enroll_all: true, department: dept } });
    setEnrolled(d.enrolled || []);
    setAvailable(d.available || []);
  };
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
                ...
                {enrolledDepts.map((dept) => (
                  <Box key={dept} className="flex items-center gap-1">
                    <Button
                      ...
                      className={cn("h-7 text-xs px-2.5", activeDept === dept && "bg-blue-50 border-blue-400 text-blue-700")}
              ...
              {filteredEnrolled.length === 0 ? (
                ...
                <Box className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {filteredEnrolled.map((u) => (
                    <Box key={u.id} className="flex items-center gap-3 p-2.5 rounded-lg border bg-gray-50/50">
                      ...
                      {u.department && (
                        <Badge variant="outline" className="text-[10px] border-gray-200 text-gray-500 shrink-0">{u.department}</Badge>
                      )}
                      <Button
                        variant="ghost" size="sm"
                        className="h-7 text-xs text-red-500 hover:bg-red-50 hover:text-red-600 shrink-0"
              ...
              {filteredAvailable.length === 0 ? (
                ...
                <Box className="space-y-2 max-h-52 overflow-y-auto pr-1">
                  {filteredAvailable.map((u) => (
                    <Box key={u.id} className="flex items-center gap-3 p-2.5 rounded-lg border hover:bg-gray-50 transition-colors">
                      <Avatar className="h-8 w-8 shrink-0">
                        <AvatarFallback className={`text-xs font-bold text-white ${avatarColor(u.id)}`}>
                          {avatarInitials(u.first_name, u.last_name)}
                        </AvatarFallback>
                      ...
                      <Button
                        variant="outline" size="sm"
                        className="h-7 text-xs text-blue-600 border-blue-200 hover:bg-blue-50 shrink-0"
        ...
        <DialogFooter>
          ...
          <Button className="bg-blue-500 hover:bg-blue-600 text-white" onClick={() => { onRosterSaved(); onClose(); }}>
            Done — Save Roster
          </Button>
        </DialogFooter>
      </DialogContent>
...
function SessionsTab({
  ...
  const statCards = [
    { icon: CalendarCheck, value: sessions.length,                                    label: "Total Sessions",  sub: "All time",           iconBg: "bg-blue-100",    iconColor: "text-blue-600",    circle: "bg-blue-50"    },
    { icon: AlertCircle,   value: sessions.filter((s) => s.status === "upcoming").length,  label: "Upcoming",        sub: "Scheduled sessions", iconBg: "bg-amber-100",   iconColor: "text-amber-600",   circle: "bg-amber-50"   },
    { icon: CheckCircle2,  value: sessions.filter((s) => s.status === "completed").length, label: "Completed",       sub: "Sessions done",      iconBg: "bg-emerald-100", iconColor: "text-emerald-600", circle: "bg-emerald-50" },
    { icon: Users,         value: totalRegistered,                                    label: "Total Registered", sub: "Across all sessions",iconBg: "bg-violet-100",  iconColor: "text-violet-600",  circle: "bg-violet-50"  },
  ];
  ...
  return (
    <Box className="space-y-5">
      ...
      <Box className="flex items-center gap-3 flex-wrap">
        ...
        <Button className="h-10 bg-blue-500 hover:bg-blue-600 text-white gap-1.5 shrink-0 px-5 text-sm" onClick={openCreate}>
                  ...
                  <Box className="flex items-start gap-4">
                    ...
                    <Box className="flex-1 min-w-0 space-y-2">
                      ...
                      <Box className="flex items-center gap-2 flex-wrap">
                        ...
                        {s.course_name && (
                          <Badge variant="outline" className="text-[11px] font-medium border-indigo-200 text-indigo-700 bg-indigo-50 flex items-center gap-1">
                    ...
                    <Box className="shrink-0 flex flex-col items-end gap-2 ml-2">
                      ...
                      {isCompleted && s.roster_count > 0 && (
                        <Text as="p" className="text-sm font-bold text-emerald-600">
                          Attendance{" "}
                      ...
                      <Box className="flex items-center gap-1.5 flex-wrap justify-end">
                        ...
                        {isUpcoming && (
                          <Button variant="outline" size="sm" className="h-7 text-xs px-2.5 text-red-500 border-red-200 hover:bg-red-50"
      ...
      <AlertDialog open={!!cancelTarget} onOpenChange={(o) => { if (!o) setCancelTarget(null); }}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleCancel} disabled={cancelling} className="bg-red-500 hover:bg-red-600 text-white">
...
function MarkAttendanceTab({ sessions, initialSessionId, token }) {
  ...
  useEffect(() => {
    ...
    setLoadingAtt(true);
    apiClient(`/api/admin/sessions/${selectedId}/attendance`, { token })
      .then((d) => {
  ...
  const statusCls   = selectedSession?.status === "upcoming" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700";
  ...
  return (
    <Box className="space-y-5">
            ...
            <Box className="grid grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              <Box className="flex gap-4">
                ...
                <Box>
                  ...
                  <Text as="p" className={cn("text-sm font-semibold", markedCount === 0 ? "text-amber-600" : markedCount === localRows.length ? "text-emerald-600" : "text-blue-600")}>
      ...
      {/* Locked banner */}
      {isLocked && (
        <Box className="flex items-center gap-2 px-4 py-3 rounded-lg bg-emerald-50 border border-emerald-200">
          <Lock className="h-4 w-4 text-emerald-600 shrink-0" />
          <Text as="p" className="text-sm text-emerald-700 font-medium">
            Attendance locked ·{" "}
      ...
      {!isLocked && localRows.length > 0 && (
        <Box className="flex items-center justify-end gap-3">
          ...
          <Button className="bg-emerald-600 hover:bg-emerald-700 text-white gap-2" onClick={() => handleSave(true)} disabled={saving}>
...
function AttendanceReportsTab({ sessions, token }) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      sessions.map((s) =>
        apiClient(`/api/admin/sessions/${s.id}/attendance`, { token })
  ...
  const reportStatCards = [
    { icon: CalendarCheck, value: sessions.length,   label: "Total Sessions",    iconBg: "bg-blue-100",    iconColor: "text-blue-600",    circle: "bg-blue-50"    },
    { icon: Users,         value: totalRegistered,   label: "Total Registered",  iconBg: "bg-violet-100",  iconColor: "text-violet-600",  circle: "bg-violet-50"  },
    { icon: CheckCircle2,  value: `${overallPct}%`,  label: "Overall Attendance",iconBg: "bg-emerald-100", iconColor: "text-emerald-600", circle: "bg-emerald-50" },
    { icon: BarChart3,     value: `${avgPerSession}%`,label: "Avg per Session",   iconBg: "bg-orange-100",  iconColor: "text-orange-600",  circle: "bg-orange-50"  },
  ];
                ...
                <CardContent className="px-4 py-3">
                  <Box className="flex items-start justify-between gap-4">
                      ...
                      {hasData ? (
                        <Box className="space-y-1.5">
                          <Text as="p" className="text-xs text-muted-foreground">
                            <Text as="span" className="text-emerald-600 font-medium">✓ Present: {presentN}</Text>
                            {"  "}
                            <Text as="span" className="text-red-500 font-medium">✗ Absent: {absentN}</Text>
                            {"  "}
                          ...
                          <Box className="h-1.5 bg-muted rounded-full overflow-hidden">
                            <Box className="h-full rounded-full bg-emerald-500" style={{ width: `${barWidth}%` }} />
                          </Box>
                        </Box>
                      ) : (
                        <Box className="flex items-center gap-1.5">
                    ...
                    <Box className="shrink-0 text-right">
                      ...
                      {attPct !== null && (
                        <Text as="p" className="text-base font-extrabold text-emerald-600">{attPct}%</Text>
                      )}
                      {attPct === null && (
...
export function AdminSessionsContent() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      apiClient("/api/admin/courses",   { token }),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        const d = await apiClient(`/api/admin/sessions/${editTarget.id}`, { method: "PUT", token, body });
        ...
        const d = await apiClient("/api/admin/sessions", { method: "POST", token, body });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiClient(`/api/admin/sessions/${deleteTarget.id}`, { method: "DELETE", token });
    ...
    <Box className="space-y-5">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <Box className="space-y-5 py-2">
            <Box className="space-y-2">
              <Label className="text-sm font-medium">Session Title <Text as="span" className="text-red-500">*</Text></Label>
                  ...
                  {["ILT", "Virtual"].map((type) => (
                    <Button key={type} type="button" variant="outline" size="sm"
                      className={cn("flex-1 h-10 text-sm font-medium transition-colors", form.session_type === type
                        ? type === "ILT" ? "bg-blue-50 border-blue-400 text-blue-700" : "bg-emerald-50 border-emerald-400 text-emerald-700"
            ...
            <Box className="grid grid-cols-2 gap-4">
              <Box className="space-y-2">
                <Label className="text-sm font-medium">Trainer / Facilitator <Text as="span" className="text-red-500">*</Text></Label>
            ...
            <Box className="space-y-2">
              <Label className="text-sm font-medium">Venue / Platform URL <Text as="span" className="text-red-500">*</Text></Label>
            ...
            <Box className="grid grid-cols-2 gap-4">
              <Box className="space-y-2">
                <Label className="text-sm font-medium">Date <Text as="span" className="text-red-500">*</Text></Label>
                <Input type="date" value={form.date} onChange={set("date")} className="h-10" />
              </Box>
              <Box className="space-y-2">
                <Label className="text-sm font-medium">Start Time <Text as="span" className="text-red-500">*</Text></Label>
            ...
            <Box className="grid grid-cols-2 gap-4">
              <Box className="space-y-2">
                <Label className="text-sm font-medium">End Time <Text as="span" className="text-red-500">*</Text></Label>
