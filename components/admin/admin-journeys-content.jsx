  ...
  return (
    <Box className="space-y-5">
      ...
      <Box className="flex items-center justify-between">
        ...
        <Button className="h-10 bg-blue-500 hover:bg-blue-600 text-white gap-2 px-5" onClick={openCreate}>
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <Box className="flex items-start justify-between gap-3">
                  <Box className="flex-1 min-w-0">
                    <Box className="flex items-center gap-2 flex-wrap mb-1">
                      {j.tag && (
                        <Badge className="text-[11px] bg-blue-50 text-blue-700 border border-blue-200 font-medium">
                          <Tag className="h-3 w-3 mr-1" />{j.tag}
                        </Badge>
                      )}
                      <Badge className={`text-[11px] border font-medium ${j.status === "Active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-50 text-gray-500 border-gray-200"}`}>
                        {j.status}
                      </Badge>
                    </Box>
                ...
                {j.skills.length > 0 && (
                  <Box className="flex flex-wrap gap-2">
                    {j.skills.map((s) => (
                      <Badge key={s} className="text-[11px] bg-gray-100 text-gray-600 border-0 font-normal">{s}</Badge>
                    ))}
                  </Box>
                )}
                ...
                <Box className="flex items-center gap-6 flex-wrap">
                  <Box>
                    <Text as="p" className="text-xl font-extrabold text-blue-600 leading-none">{j.enrolled}</Text>
                    <Text as="p" className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mt-0.5">Enrolled</Text>
                  </Box>
                  <Box>
                    <Text as="p" className="text-xl font-extrabold text-amber-500 leading-none">{j.completedPct}%</Text>
                ...
                <Box className="flex items-center gap-3 flex-wrap">
                  {j.courses.map((c, idx) => (
                    <Box key={c} className="flex items-center gap-1.5 text-sm text-muted-foreground">
                      <Box className="w-5 h-5 rounded-full bg-blue-500 text-white text-[10px] font-bold flex items-center justify-center shrink-0">
                        {idx + 1}
                      </Box>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <Box className="space-y-5 py-2">
            ...
            <Box className="space-y-1.5">
              <Label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
                Journey Title <Text as="span" className="text-red-500">*</Text>
              </Label>
              <Input
            ...
            <Box className="space-y-2">
              <Label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
                Courses in Sequence <Text as="span" className="text-red-500">*</Text>
              </Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} className="bg-blue-500 hover:bg-blue-600 text-white px-6">
              Create Journey
            </Button>
          </DialogFooter>
        </DialogContent>
