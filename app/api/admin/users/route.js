...
export async function GET(request) {
  ...
  const users = (await db.execute(`
    SELECT u.id, u.first_name, u.last_name, u.email, u.department, u.location, u.job_role, u.is_active, u.created_at,
...
export async function POST(request) {
  ...
  const { first_name, last_name, email, password, department, location, job_role } = await request.json();
  ...
  const result = await db.execute({
    sql: `INSERT INTO users (first_name, last_name, email, password, role, department, location, job_role) VALUES (?,?,?,?,'learner',?,?,?)`,
    args: [first_name.trim(), last_name.trim(), email.toLowerCase().trim(), hashPassword(password), department?.trim() || null, location?.trim() || null, job_role?.trim() || null],
  });
