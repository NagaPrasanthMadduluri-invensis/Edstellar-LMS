...
export async function POST(request, { params }) {
  ...
  const assessment = (await db.execute({ sql: "SELECT * FROM assessments WHERE id = ? AND is_active = 1", args: [assessmentId] })).rows[0];
  ...
  const questions = (await db.execute({
    sql: `SELECT aq.*, ao.id as correct_option_id
  ...
  const attemptResult = await db.execute({
    sql: `INSERT INTO user_assessment_attempts (user_id, assessment_id, score, total_questions, percentage, is_passed) VALUES (?,?,?,?,?,?)`,
  ...
  for (const a of scoredAnswers) {
    await db.execute({
      sql: `INSERT INTO user_assessment_answers (attempt_id, question_id, selected_option_id, is_correct) VALUES (?,?,?,?)`,
      args: [attemptId, a.question_id, a.selected_option_id, a.is_correct],
    });
  }
