...
// Normalised in one place — a trailing slash on the env var would make every
// endpoint `//api/...`, which Nest answers with 404. See lib/server-url.js.
import { SERVER_URL as SERVER_BASE_URL } from "@/lib/server-url";
...
export async function apiClient(endpoint, options = {}) {
  ...
  if (contentType && !contentType.includes("application/json")) {
    ...
    /**
     * An HTML body is never a legitimate download — it means the request was
     * answered by something that is not the API: a dev server whose catch-all
     * route redirected to /login, a proxy error page, a misconfigured
     * NEXT_PUBLIC_SERVER_URL pointing back at the frontend itself.
     *
     * Handing the Response back to the caller made it read `undefined` off an
     * object it thought was JSON, so the failure surfaced far from its cause —
     * `normalizeUser(data.user)` returned null and login died on
     * "Cannot read properties of null (reading 'role')". Fail here, where the
     * reason is still known.
     */
    if (contentType.includes("text/html")) {
      throw new ApiError(
        `${endpoint} was answered with HTML instead of JSON by ` +
          `${SERVER_BASE_URL || "this origin"}, so the request never reached the ` +
          "API. Check that the API server is running and that " +
          "NEXT_PUBLIC_SERVER_URL points at it.",
        res.status,
      );
    }
    return res;
