 ...
 */
/**
 * Resolves the session AND says why it could not be resolved.
 *
 * The `why` is the whole point. This used to return a bare `null` for three
 * unrelated situations, and the caller then told the user their session had
 * expired for all three. One of them — the API being unreachable from THIS
 * process — is not about the user's session at all: their cookie is perfectly
 * valid, the server simply cannot ask about it. Saying "your session has
 * ended" there sends whoever is debugging after sessions, tokens and
 * permissions when the actual fault is a network hop. It cost a long
 * debugging session to find that out.
 *
 *   'no-cookie'    nobody is signed in — an ordinary first visit
 *   'rejected'     the API answered, and said this token is not valid
 *   'unreachable'  the API could not be asked at all
 */
async function resolveSession() {
  const cookieStore = await cookies();
  const token = cookieStore.get(TOKEN_COOKIE)?.value;
  if (!token) return { user: null, why: "no-cookie" };
  ...
  try {
    ...
    // The API was reached and refused the token — a real expiry, a rotated
    // secret, a deactivated account.
    if (!res.ok) return { user: null, why: "rejected", status: res.status };

    const { user } = await res.json();
    if (!user) return { user: null, why: "rejected", status: res.status };
    ...
    return { why: null, user: {
      ...
      /**
       * Set only when a platform admin is working INSIDE this tenant
       * (`{ byName, organizationName }`, otherwise null).
       *
       * Resolved here rather than client-side because the banner has to be in
       * the first paint. The realistic failure this guards against is a super
       * admin forgetting whose account they are in and changing the wrong
       * tenant's data — a banner that appears a moment after the page does is
       * a banner they can act before.
       */
      impersonation: user.impersonation
        ? {
            byName: user.impersonation.by_name,
            organizationName: user.impersonation.organization_name,
          }
        : null,
    } };
  } catch (error) {
    ...
    return { user: null, why: "unreachable", error };
  }
}

/** The user, or null. Unchanged for callers that only need the user. */
 ...
 */
/**
 * The user, or a redirect to /login that says what actually went wrong.
 *
 * One helper instead of `if (!user) redirect(...)` repeated in five layouts,
 * so the three outcomes cannot drift apart between portals:
 *
 *   no cookie    -> /login                    a first visit. No alarming copy.
 *   rejected     -> /login?session=expired    genuinely signed out.
 *   unreachable  -> /login?error=unavailable  OUR fault, not theirs. Says so.
 *
 * `middleware.js` needs one of these markers to render the form at all — with
 * a cookie present and no marker it bounces /login back to /, which is an
 * infinite redirect. See the comment there.
 */
export async function requireSession() {
  const { user, why } = await resolveSession();
  if (user) return user;
